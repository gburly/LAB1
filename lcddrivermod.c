...
void DelayUs(unsigned int usDelay) {
    ...
    TMR2 = 0;
    PR2 = 14;                       //set PR1 to 14 for one microsecond delay
    IFS0bits.T2IF = 0;              //flag
    PR2 = usDelay*PR2;              //multiply the user delay input by PR2
    T2CON = 0x8000;                 //timer on
    while(IFS0bits.T2IF == 0);      //timer stays on until flag goes to 1
    IFS0bits.T2IF = 0;
    T2CON = 0;                      //turn timer off
...
//        EnableLCD(LCD_WRITE_CONTROL, 100);
	...
	// TODO: Display On/Off Control
        WriteLCD(0x0C, LCD_WRITE_CONTROL, 40);   // Turn Display (D) Off
...
void LCDClear(void) {
    ...
    WriteLCD(0x01, LCD_WRITE_CONTROL, 1640); //Clears the display with highest delay
